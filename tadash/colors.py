...
# 'gist_earth', 'gist_earth_r', 'gist_gray', 'gist_gray_r', 'gist_grey', 'gist_heat', 'gist_heat_r', 'gist_ncar',
# 'gist_ncar_r', 'gist_rainbow', 'gist_rainbow_r', 'gist_stern', 'gist_stern_r', 'gist_yarg', 'gist_yarg_r',
# 'gist_yerg', 'gnuplot', 'gnuplot2', 'gnuplot2_r', 'gnuplot_r', 'gray', 'gray_r', 'grey', 'hot', 'hot_r', 'hsv',
...
import matplotlib.pyplot as plt
import matplotlib.colors as mcolors
def color_palette(values: list, palette_name: str ='viridis', reverse: bool =True):
    cmap = plt.get_cmap(palette_name)
    colors = [cmap(i / (len(values) - 1)) for i in range(len(values))]
    gradient_palette =  [mcolors.rgb2hex(color) for color in reversed(colors)] if reverse else [mcolors.rgb2hex(color) for color in colors]
    return {value: gradient_palette[i] for i, value in enumerate(values)}
